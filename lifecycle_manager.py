#!/usr/bin/env python3
"""
Research Assistant Plugin Lifecycle Manager

This script handles install/update/delete operations for the Research Assistant plugin
...
class ResearchAssistantLifecycleManager(BaseLifecycleManager):
    """Lifecycle manager for Research Assistant plugin"""
    ...
    def __init__(self, plugins_base_dir: str = None):
        ...
        self.plugin_data = {
            ...
            "author": "BrainDrive",
            "compatibility": "1.0.0",
            "scope": "ResearchAssistant",
            "bundle_method": "webpack",
            "bundle_location": "dist/remoteEntry.js",
            "is_local": False,
            "long_description": "A BrainDrive plugin that helps triage articles and resources for relevance to active projects. Paste article text, select a project, and get AI-powered analysis with recommendations to integrate now, save for later, or skip.",
            "plugin_slug": "ResearchAssistant",
            "source_type": "github",
            "source_url": "https://github.com/davewaring/ResearchAssistant-Plugin",
            "update_check_url": "https://api.github.com/repos/davewaring/ResearchAssistant-Plugin/releases/latest",
        ...
        self.module_data = [
            {
                "name": "ResearchAssistantModule",  # Must differ from plugin name
                "display_name": "Research Assistant",
                "description": "Analyze articles for relevance to your projects",
                "icon": "FileSearch",
                "category": "productivity",
                "priority": 1,
                "props": {
                    "title": "Research Assistant",
                    "description": "Paste article text to analyze relevance to your projects",
                    "config": {
                ...
                "config_fields": {
                    ...
                    "library_path": {
                        "type": "text",
                        "description": "Path to BrainDrive-Library",
                ...
                "tags": ["research", "articles", "productivity", "ai"]
            }
        ]
        
        # Initialize base class with required parameters
        logger.info(f"ResearchAssistant: plugins_base_dir - {plugins_base_dir}")
        if plugins_base_dir:
        ...
        logger.info(f"ResearchAssistant: shared_path - {shared_path}")
        super().__init__(
    ...
    async def _perform_user_installation(self, user_id: str, db: AsyncSession, shared_plugin_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"ResearchAssistant: User installation completed for {user_id}")
            return {
                'success': True,
            ...
            logger.error(f"ResearchAssistant: User installation failed for {user_id}: {e}")
    ...
    async def _perform_user_uninstallation(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"ResearchAssistant: User uninstallation completed for {user_id}")
            return {
                'success': True,
                'plugin_id': plugin_id,
            ...
            logger.error(f"ResearchAssistant: User uninstallation failed for {user_id}: {e}")
            return {'success': False, 'error': str(e)}
    ...
    async def _copy_plugin_files_impl(self, user_id: str, target_dir: Path, update: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"ResearchAssistant: Copied {len(copied_files)} files/directories to {target_dir}")
            return {'success': True, 'copied_files': copied_files}
            
        except Exception as e:
            logger.error(f"ResearchAssistant: Error copying plugin files: {e}")
            return {'success': False, 'error': str(e)}
    ...
    async def _validate_installation_impl(self, user_id: str, plugin_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if missing_files:
                return {
                    'valid': False,
                    'error': f"ResearchAssistant: Missing required files: {', '.join(missing_files)}"
                }
            ...
            try:
                ...
                for field in required_fields:
                    if field not in package_data:
                        return {
                            'valid': False,
                            'error': f'ResearchAssistant: package.json missing required field: {field}'
                        }
                        
            except (json.JSONDecodeError, FileNotFoundError) as e:
                return {
                    'valid': False,
                    'error': f'ResearchAssistant: Invalid or missing package.json: {e}'
                }
            ...
            if bundle_path.stat().st_size == 0:
                return {
                    'valid': False,
                    'error': 'ResearchAssistant: Bundle file (remoteEntry.js) is empty'
                }
            
            logger.info(f"ResearchAssistant: Installation validation passed for user {user_id}")
            return {'valid': True}
            
        except Exception as e:
            logger.error(f"ResearchAssistant: Error validating installation: {e}")
    ...
    async def _get_plugin_health_impl(self, user_id: str, plugin_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"ResearchAssistant: Error checking plugin health: {e}")
            return {
                'healthy': False,
                'details': {'error': str(e)}
            }
    ...
    async def _check_existing_plugin(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            plugin_slug = self.plugin_data['plugin_slug']
            logger.info(f"ResearchAssistant: Checking for existing plugin - user_id: {user_id}, plugin_slug: {plugin_slug}")
            
            # First test database connectivity
            test_query = text("SELECT COUNT(*) as count FROM plugin")
            test_result = await db.execute(test_query)
            test_row = test_result.fetchone()
            logger.info(f"ResearchAssistant: Database connectivity test - total plugins: {test_row.count}")
            ...
            logger.info(f"ResearchAssistant: Executing query with params: {query_params}")
            
            result = await db.execute(plugin_query, query_params)
            
            plugin_row = result.fetchone()
            logger.info(f"ResearchAssistant: Query result: {plugin_row}")
            if plugin_row:
                logger.info(f"ResearchAssistant: Found existing plugin - id: {plugin_row.id}, name: {plugin_row.name}")
                return {
                    'exists': True,
                ...
                logger.warning(f"ResearchAssistant: No plugin found for user_id: {user_id}, plugin_slug: {plugin_slug}")
                ...
                if debug_rows:
                    logger.info(f"ResearchAssistant: User has {len(debug_rows)} other plugins:")
                    for row in debug_rows:
                        logger.info(f"  - {row.plugin_slug} (id: {row.id})")
                else:
                    logger.info(f"ResearchAssistant: User has no plugins installed")
                
                return {'exists': False}
                
        except Exception as e:
            logger.error(f"ResearchAssistant: Error checking existing plugin: {e}")
    ...
    async def _create_database_records(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"ResearchAssistant: Creating database records - user_id: {user_id}, plugin_slug: {plugin_slug}, plugin_id: {plugin_id}")
            ...
            await db.commit()
            logger.info(f"ResearchAssistant: Database transaction committed successfully")
            ...
            if verify_row:
                logger.info(f"ResearchAssistant: Successfully created and verified database records for plugin {plugin_id} with {len(modules_created)} modules")
            else:
                logger.error(f"ResearchAssistant: Plugin creation appeared to succeed but verification failed for plugin_id: {plugin_id}")
    ...
    async def _export_user_data(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"ResearchAssistant: Exported user data for {user_id}")
            return {'success': True, 'user_data': user_data}
            
        except Exception as e:
            logger.error(f"ResearchAssistant: Error exporting user data: {e}")
            return {'success': False, 'error': str(e)}
    ...
    async def _import_user_data(self, user_id: str, db: AsyncSession, user_data: Dict[str, Any]):
        ...
        try:
            if not user_data:
                logger.info(f"ResearchAssistant: No user data to import for {user_id}")
                return
            ...
            logger.info(f"ResearchAssistant: Imported user data for {user_id}")
            
        except Exception as e:
            logger.error(f"ResearchAssistant: Error importing user data: {e}")
            raise
    ...
    async def install_plugin(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            logger.info(f"ResearchAssistant: Starting installation for user {user_id}")
            
            # Check if plugin is already installed for this user
            existing_check = await self._check_existing_plugin(user_id, db)
            if existing_check['exists']:
                logger.warning(f"ResearchAssistant: Plugin already installed for user {user_id}")
                return {
                    'success': False,
            ...
            logger.info(f"ResearchAssistant: Created shared directory: {shared_path}")

            # Copy plugin files to the shared directory first
            copy_result = await self._copy_plugin_files_impl(user_id, shared_path)
            if not copy_result['success']:
                logger.error(f"ResearchAssistant: File copying failed: {copy_result.get('error')}")
                return copy_result

            logger.info(f"ResearchAssistant: Files copied successfully, proceeding with database installation")
            ...
            try:
                ...
                if result.get('success'):
                    ...
                    if not verify_check['exists']:
                        logger.error(f"ResearchAssistant: Installation appeared successful but verification failed")
                        return {'success': False, 'error': 'Installation verification failed'}
                    
                    logger.info(f"ResearchAssistant: Installation verified successfully for user {user_id}")
                    result.update({
                        'plugin_slug': self.plugin_data['plugin_slug'],
                        'plugin_name': self.plugin_data['name']
                    })
                else:
                    logger.error(f"ResearchAssistant: Database installation failed: {result.get('error')}")
                
                return result
                
            except Exception as db_error:
                logger.error(f"ResearchAssistant: Database operation failed: {db_error}")
            ...
            logger.error(f"ResearchAssistant: Install plugin failed: {e}")
            return {'success': False, 'error': str(e)}
    ...
    async def delete_plugin(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            logger.info(f"ResearchAssistant: Starting deletion for user {user_id}")
            ...
            if result.get('success'):
                logger.info(f"ResearchAssistant: Successfully deleted plugin for user {user_id}")
            else:
                logger.error(f"ResearchAssistant: Deletion failed: {result.get('error')}")
            
            return result
        except Exception as e:
            logger.error(f"ResearchAssistant: Delete plugin failed: {e}")
            return {'success': False, 'error': str(e)}
    ...
    async def get_plugin_status(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"ResearchAssistant: Error checking plugin status: {e}")
            return {'exists': False, 'status': 'error', 'error': str(e)}
    
    async def update_plugin(self, user_id: str, db: AsyncSession, new_version_manager: 'ResearchAssistantLifecycleManager') -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"ResearchAssistant: Plugin updated successfully for user {user_id}")
            return {
                'success': True,
            ...
            logger.error(f"ResearchAssistant: Plugin update failed for user {user_id}: {e}")
            return {'success': False, 'error': str(e)}
...
async def install_plugin(user_id: str, db: AsyncSession, plugins_base_dir: str = None) -> Dict[str, Any]:
    manager = ResearchAssistantLifecycleManager(plugins_base_dir)
...
async def update_plugin(user_id: str, db: AsyncSession, new_version_manager: 'ResearchAssistantLifecycleManager', plugins_base_dir: str = None) -> Dict[str, Any]:
    old_manager = ResearchAssistantLifecycleManager(plugins_base_dir)
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        manager = ResearchAssistantLifecycleManager()
